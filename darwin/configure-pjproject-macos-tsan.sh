...
export CFLAGS="${CFLAGS} -fsanitize=thread -isysroot $SDKPATH -mmacosx-version-min=11 -DPJ_SDK_NAME=\"\\\"${PJ_SDK_NAME}\\\"\""
export CXXFLAGS="${CXXFLAGS} -fsanitize=thread"
export LDFLAGS="-fsanitize=thread -isysroot ${SDKPATH}"
